// bom-mobile/src/features/sales/pickers/RmPickerSheet.tsx
...
export function RmPickerSheet({ visible, onPick, onClose, onCreateNew }: Props) {
  const { user } = useAuth();
  const [search, setSearch] = useState("");
  const { data, isLoading } = useItems({ type: "RawMaterial", branchId: user?.branchId ?? undefined });
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={{
        ...
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
          <Text style={{ fontWeight: "600", fontSize: 16 }}>Select raw material</Text>
        ...
        {isLoading ? <ActivityIndicator style={{ marginTop: 20 }} /> : (
          <FlatList
            ...
            ListEmptyComponent={<Text style={{ padding: 16, color: "#94a3b8" }}>No raw materials found.</Text>}
          />
        )}
      </View>
    </Modal>
