...
type MdTab = "queue" | "in-flight" | "signed" | "rejected";

const VALID_TABS: MdTab[] = ["queue", "in-flight", "signed", "rejected"];
...
const TAB_STATUSES: Record<MdTab, V3RequisitionStatus[]> = {
  queue: ["MdPricing", "MdFinalSign"],
  "in-flight": ["Costing", "CustomerConfirm"],
  signed: ["Signed"],
  rejected: ["Rejected", "Cancelled"],
};
...
export function MdListPage() {
  const navigate = useNavigate();
  const [params, setParams] = useSearchParams();
  const tabParam = params.get("tab") as MdTab | null;
  const tab: MdTab = tabParam && VALID_TABS.includes(tabParam) ? tabParam : "queue";
...
function emptyHintFor(tab: MdTab): string {
  switch (tab) {
    case "queue":
